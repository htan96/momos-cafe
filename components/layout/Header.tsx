  ...
  return (
    ...
    >
      <div className="max-w-[1140px] mx-auto px-5 h-16 flex relative items-center justify-center md:justify-between">
        <div className="momos-header-mobile-only absolute left-5 top-1/2 -translate-y-1/2 flex items-center gap-1.5 md:hidden">
          <button
            type="button"
            className="inline-flex h-10 w-10 items-center justify-center rounded-full text-teal-dark border border-cream-dark/80 hover:bg-teal/10 transition-colors"
            aria-label={mobileMenuOpen ? "Close site menu" : "Open site menu"}
            aria-expanded={mobileMenuOpen}
            onClick={() => setMobileMenuOpen((o) => !o)}
          >
            <NavMenuIcon className="w-6 h-6" aria-hidden />
          </button>
          <HeaderMobileAccount />
        </div>
        ...
        <div className="momos-header-mobile-only absolute right-5 top-1/2 -translate-y-1/2 flex items-center md:hidden">
          <HeaderMobileCart />
        </div>

        <nav
          className="momos-header-desktop-nav hidden md:flex items-center gap-1.5"
          aria-label="Primary"
        >
